...
CI-имя класса — `CIRobot`. В языке EME-L класс регистрируется под именем `Robot`. Вспомогательный класс `CRobotPopupStyle` (стили всплывающих окон) и `CRobotDataContainer` (контейнеры данных) — отдельные классы, создаваемые методами `Robot`, а не сам скриптовый класс `Robot`.
...
| `FocusPopup(place, text)` | place: String, text: String | Empty | Выводит всплывающую подсказку рядом с элементом интерфейса. Продолжительность рассчитывается так же, как у `Popup(style, text)` — `задержка + количество слов × задержка / 3`. |
